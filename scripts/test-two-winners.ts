/**
 * TEST: Two Winners (Same Token)
 * 
 * This test verifies that when 2 players enter with the same winning token,
 * they each receive 50% of the 90% rewards (45% each).
 * 
 * Strategy:
 * - 2 players enter with FARTCOIN (will be the winner)
 * - 8 players enter with different tokens
 * - Set fake end price for FARTCOIN to ensure it wins (high volatility)
 * - Both winners claim and verify they get 50% each
 */
...
async function main() {
  console.log("\n" + "═".repeat(80));
  console.log("🧪 TEST: TWO WINNERS (SAME TOKEN)");
  console.log("═".repeat(80));
  console.log("   Expected: 2 winners each get 50% of the 90% rewards (45% each)");
  ...
  // ================================================================
  // ENTER 10 PLAYERS - 2 with FARTCOIN, 8 with different tokens
  // ================================================================
  console.log("═".repeat(80));
  console.log("ENTERING 10 PLAYERS (2 WITH FARTCOIN)");
  console.log("═".repeat(80) + "\n");
  ...
  // Asset distribution: 
  // Player 1,2: FARTCOIN (8) - will be winners
  // Player 3-10: SOL(0), TRUMP(1), PUMP(2), BONK(3), JUP(4), PENGU(5), PYTH(6), HNT(7)
  const assetAssignment = [8, 8, 0, 1, 2, 3, 4, 5, 6, 7];
  ...
  console.log(`   🏆 FARTCOIN players: ${fartcoinAsset.playerCount} (both will be winners)\n`);
  ...
  if (winners.length !== 2) {
    console.log(`\n⚠️  Expected 2 winners, got ${winners.length}`);
  }

  // ================================================================
  // BOTH WINNERS CLAIM THEIR OWN TOKENS
  // ================================================================
  console.log("\n" + "═".repeat(80));
  ...
  // ================================================================
  // BOTH WINNERS CLAIM FROM LOSERS (50% EACH OF 90%)
  // ================================================================
  console.log("\n" + "═".repeat(80));
  console.log("WINNERS CLAIM FROM LOSERS (50% each of 90% = 45% each)");
  console.log("═".repeat(80) + "\n");

  const winningAssetPda = arenaAssetPdas[winningAsset];

  // Track claims for verification
  const claimsSummary: { [loserAsset: string]: { winner1: number; winner2: number; treasury: number } } = {};

  for (const loser of losers) {
    const loserAssetName = ASSET_NAMES[loser.assetIndex];
    claimsSummary[loserAssetName] = { winner1: 0, winner2: 0, treasury: 0 };
  ...
  // ================================================================
  console.log("\n" + "═".repeat(80));
  console.log("VERIFICATION: 50/50 SPLIT CHECK");
  console.log("═".repeat(80) + "\n");

  console.log("Token          | Winner 1   | Winner 2   | Ratio   | Treasury");
  console.log("─".repeat(70));

  let allSplitsCorrect = true;

  for (const [assetName, claims] of Object.entries(claimsSummary)) {
    const total = claims.winner1 + claims.winner2;
    const ratio1 = total > 0 ? (claims.winner1 / total * 100).toFixed(1) : "0.0";
    const ratio2 = total > 0 ? (claims.winner2 / total * 100).toFixed(1) : "0.0";
    
    const isCorrect = Math.abs(claims.winner1 - claims.winner2) < 0.001 || 
                      (total > 0 && Math.abs(parseFloat(ratio1) - 50) < 1);
    
    const status = isCorrect ? "✅" : "⚠️";
    
    console.log(`${assetName.padEnd(14)} | ${claims.winner1.toFixed(4).padStart(10)} | ${claims.winner2.toFixed(4).padStart(10)} | ${ratio1}/${ratio2} ${status} | ${claims.treasury.toFixed(4)}`);
  ...
  const winnersCorrect = winners.length === 2;
  const winningAssetCorrect = winningAsset === WINNING_ASSET_INDEX;

  console.log(`   Number of winners:    ${winnersCorrect ? "2 ✅" : `${winners.length} ❌`}`);
  console.log(`   Winning asset:        ${ASSET_NAMES[winningAsset]} ${winningAssetCorrect ? "✅" : "⚠️"}`);
  console.log(`   50/50 split correct:  ${allSplitsCorrect ? "YES ✅" : "NEEDS REVIEW ⚠️"}\n`);

  if (winnersCorrect && allSplitsCorrect) {
    console.log("🎉 TEST PASSED!");
    console.log("   ✅ 2 players with same winning token both received rewards");
    console.log("   ✅ Each winner received ~50% of the 90% loser rewards (45% each)");
